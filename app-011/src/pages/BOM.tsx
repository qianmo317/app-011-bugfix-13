...
import { calcMaterials, calcPaintBuckets, calcRoomWall } from '../utils/materialCalc';
import type { MatSpec, Room, Opening } from '../types';
  ...
  return (
    <div className="card">
      ...
      {rooms.map((room) => {
        ...
        const buckets = calcPaintBuckets(wall.netAreaM2, paintMat.coverage || 12);
        const primerBuckets = primerMat ? calcPaintBuckets(wall.netAreaM2, primerMat.coverage || 14) : 0;
        ...
        return (
          <div key={room.id} style={{ marginBottom: 12, padding: 12, background: '#f8f9fa', borderRadius: 4 }}>
            ...
            <div style={{ fontSize: 13, color: '#666' }}>
              周长{wall.perimMm.toFixed(0)}mm × 层高{room.heightMm}mm = {wall.wallAreaMm2.toFixed(0)}mm²
              <br />
              扣门窗{wall.openingAreaMm2.toFixed(0)}mm² → 净面积{wall.netAreaM2.toFixed(2)}m²
              <br />
